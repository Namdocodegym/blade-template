    ...
    <form action="{{ route('users.post-edit') }}" method="POST">
            ...
            <select name="group_id" class="form-control" id="">
                ...
                @if (!empty($allGroups))
                    @foreach ($allGroups as $item)
                        <option value="{{ $item->id }}" {{ old('group_id')==$item->id || $userDetail->group_id==$item->id?'selected':'' }}>
                            {{ $item->name }}</option>
                    @endforeach
                @endif
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="status" class="form-control" id="">
                <option value="0" {{ old('status')==0 || $userDetail->status==0?'selected':'' }}>inactive</option>
                <option value="1" {{ old('status')==1 || $userDetail->status==1?'selected':'' }}>active</option>
