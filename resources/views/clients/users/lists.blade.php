...
@section('content')
    ...
    <form action="" method="get" class="mb-3">
        <div class="row">
            ...
            <div class="col-4">
                <input type="search" name="keywords" class="form-control" placeholder="検索"
    ...
    <table class="table table-bordered">
        <thead>
            <tr>
                <th width="10%">番号</th>
                <th><a href="?sortBy=name&sortType={{ $sortType }}" > 名前</a></th>
                <th><a href="?sortBy=email&sortType={{ $sortType }}">メール</a></th>
                <th>グループ</th>
                <th>状態</th>
                <th width="15%"><a href="?sortBy=created_at&sortType={{ $sortType }}">時間</a></th>
                <th with="5%">編集</th>
                <th with="5%">削除</th>
            ...
            @if (!empty($usersList))
                @foreach ($usersList as $key => $item)
                    <tr>
                        ...
                        <td>
                            <a onclick="return confirm('削除しますか？')" href="{{ route('users.delete', ['id' => $item->id]) }}"
